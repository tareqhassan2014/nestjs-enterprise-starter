...
When a low-balance threshold is configured, the system SHALL emit a structured extension signal (domain event or equivalent) after a spend that leaves the wallet at or below that threshold. The signal MUST NOT itself send email or enqueue work in this change.
