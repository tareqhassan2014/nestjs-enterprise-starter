...
Who the caller is: registration and sign-in, the sessions those flows establish, and how a session is presented on a later request.

Sessions are durable records rather than self-contained tokens, so they can be enumerated and revoked with no cached-credential window. A cache may serve lookups, but the database stays authoritative — a cache outage must not deny a valid session, and a cache hit must not resurrect a revoked one. The authentication library owns its own route surface, mounted outside the versioned API so its contract does not move when the API version does.
