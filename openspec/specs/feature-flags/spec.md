...
A code-declared feature flag vocabulary evaluated at runtime: environment/bootstrap defaults with optional persisted overrides, scoped by user or organization context. Flags gate incomplete or experimental surfaces without redeploying for every toggle, but never substitute for the auth → RBAC → entitlements → throttle → usage → credits guard chain.
