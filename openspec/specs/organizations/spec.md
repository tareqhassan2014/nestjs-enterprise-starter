...
A minimal organization entity and membership model with a billing-subject resolver, so credits and plan resolution can operate on an organization's wallet and subscription instead of a user's when a request/context carries an org. This is a hook for org-primary billing, not a full multi-tenant product: no invitations UI, SSO-per-tenant, or custom domains.
