...
User↔plan or organization↔plan subscription records with monthly/yearly billing intervals, lifecycle states (`active`, `past_due`, `canceled`), effective-plan resolution with Lite fallback keyed off the active billing subject, and an authenticated current-plan read surface.
