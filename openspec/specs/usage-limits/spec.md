...
Each feature MUST have configurable daily and weekly ceilings sourced from validated application configuration, with documented defaults suitable for the starter.
...
#### Scenario: Ceiling from configuration

- **WHEN** a feature's daily ceiling is set via configuration and a user reaches that count
- **THEN** further consumes for that feature that day are rejected
